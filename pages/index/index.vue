	...
	<view class="page">
		<view class="container">
			...
			<view class="card card--green">
				<text class="card-title">初始金额设置</text>
				<view class="pill">
					<uni-icons type="wallet" size="18" color="#4CAF87"></uni-icons>
					<input class="pill-input" :class="{ 'pill-input--focused': totalFocused }" type="digit" placeholder="请输入初始金额"
						placeholder-class="placeholder-soft" v-model="totalAmount" @focus="totalFocused = true"
						@blur="totalFocused = false" />
				</view>
				<button class="primary-btn" hover-class="primary-btn--active" @click="saveInitialAmount">确认设置</button>
			</view>
			...
			<view class="card card--paper">
				...
				<view v-else class="table">
					<view v-for="item in records" :key="item.id" class="tr" @click="toggleSelected(item.id)">
				...
				<button class="add-btn" hover-class="add-btn--active" @click="openAdd">添加支出</button>
			</view>
		</view>

		<view v-if="leafAnimating" class="leaf-fall" @animationend="leafAnimating = false">
		...
		<view v-if="showAdd" class="mask" @click="closeAdd">
			<view class="modal" @click.stop>
				...
				<view class="form-row">
					...
					<input class="form-input" placeholder="例如：早餐" placeholder-class="placeholder-soft" v-model="addForm.item" />
				</view>

				<view class="form-row">
					<text class="form-label">金额</text>
					<input class="form-input form-input--right" type="digit" placeholder="请输入金额"
						placeholder-class="placeholder-soft" v-model="addForm.amount" />
				</view>

				<view class="modal-actions">
					<button class="btn btn--ghost" hover-class="btn--ghost-active" @click="closeAdd">取消</button>
					<button class="btn btn--primary" hover-class="btn--primary-active" @click="saveAdd">保存</button>
				</view>
			</view>
		</view>
	</view>
...
function normalizeRecord(r) {
	...
	const id = r.id ? String(r.id) : `${Date.now()}_${Math.random().toString(16).slice(2)}`
...
function formatAmount(value) {
	...
	return String(fixed).includes('.') ? fixed.toFixed(2).replace(/\.?0+$/, '') : String(fixed)
}
...
function saveInitialAmount() {
	uni.setStorageSync('sbk_total_amount', totalAmount.value)
...
function saveAdd() {
	...
	const amountNumber = parseFloat(String(addForm.value.amount).replace(/[^\d.]/g, ''))
	if (!itemText) {
...
function toggleSelected(id) {
	records.value = records.value.map((r) => (r && r.id === id ? Object.assign({}, r, { selected: !r.selected }) : r))
	uni.setStorageSync('sbk_records', records.value)
}
...
.page {
	min-height: 100vh;
	background: radial-gradient(circle at 18% 8%, rgba(76, 175, 135, 0.18) 0%, rgba(76, 175, 135, 0) 52%),
		radial-gradient(circle at 85% 18%, rgba(102, 194, 165, 0.16) 0%, rgba(102, 194, 165, 0) 56%), #f8fcf9;
}
...
.card--green {
	position: relative;
	overflow: hidden;
	background: linear-gradient(180deg, rgba(76, 175, 135, 0.28) 0%, rgba(102, 194, 165, 0.18) 56%, rgba(255, 255, 255, 0.78) 100%);
...
.card--green::before {
	...
	height: 360rpx;
	background: radial-gradient(circle at 20% 10%, rgba(255, 255, 255, 0.76) 0%, rgba(255, 255, 255, 0) 56%),
		radial-gradient(circle at 86% 0%, rgba(255, 255, 255, 0.46) 0%, rgba(255, 255, 255, 0) 60%);
	pointer-events: none;
}

.card--paper {
	background: rgba(255, 255, 255, 0.86);
	box-shadow: 0 24rpx 64rpx rgba(56, 142, 107, 0.10);
}
...
.pill {
	...
	box-shadow: inset 0 0 0 2rpx rgba(102, 194, 165, 0.22);
}
...
.pill-input--focused {
	border-bottom-color: rgba(76, 175, 135, 0.7);
}
...
.leaf {
	...
	background: linear-gradient(180deg, rgba(102, 194, 165, 0.95) 0%, rgba(76, 175, 135, 0.9) 100%);
	opacity: 0.92;
}
...
.form-row {
	...
	padding: 18rpx 18rpx;
	border-radius: 24rpx;
	background: rgba(102, 194, 165, 0.10);
}
...
.btn--ghost {
	background: rgba(56, 142, 107, 0.10);
	color: #388e6b;
}
